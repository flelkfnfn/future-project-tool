...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  useEffect(() => {
    ...
    channel
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'project_links', filter: `project_id=eq.${project.id}` }, (payload) => {
        const newLink = payload.new as ProjectLink;
        setLiveProject(prev => {
          if (prev.project_links.some(link => link.id === newLink.id)) return prev;
          return { ...prev, project_links: [...prev.project_links, newLink] };
        });
      })
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'project_links', filter: `project_id=eq.${project.id}` }, (payload) => {
        const deletedLink = payload.old as Partial<ProjectLink>;
        setLiveProject(prev => ({ ...prev, project_links: prev.project_links.filter(link => link.id !== deletedLink.id) }));
      })
      .subscribe();
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 w-80 flex-shrink-0 flex flex-col">
      <div className="flex justify-between items-start mb-2">
        ...
        <AuthGuardForm action={deleteProject} confirmMessage="프로젝트를 삭제하시겠습니까?">
          ...
          <button type="submit" className="text-gray-400 hover:text-red-500 transition-colors" aria-label="프로젝트 삭제">
      ...
      <div className="flex-grow space-y-2">
        ...
        <div id={`project-links-${liveProject.id}`} className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${open ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
          <div className={`overflow-hidden transition-opacity duration-300 ${open ? "opacity-100" : "opacity-0"}`}>
            {liveProject.project_links && liveProject.project_links.map((link) => (
              <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" title={link.title ? `${link.title} - ${link.url}` : link.url} className="block bg-gray-100 dark:bg-gray-700 p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors mb-2">
