...
export default async function ProjectsPage() {  
    ...
    const { data: projects, error } = await supabase.from("projects").select("id, name").order("id", { ascending: false });  
    if (error) { return <p className="text-red-500">프로젝트를 불러오는 중 오류가 발생했습니다.</p>; }  
        return (    
        <div>      
            ...
            {((projects as unknown as Project[]) ?? []).length > 0 ? (        
                ...
                {((projects as unknown as Project[]) ?? []).map((project: Project) => (            
                    ...
                    <AuthGuardForm action={deleteProject} confirmMessage="정말 삭제하시겠습니까?">                
                        ...
                        <button type="submit" className="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">삭제</button>              
                    </AuthGuardForm>            
                    </li>          
                    ))}        
                </ul>      
